          ...
          {CARD_TYPES.map((card, i) => (
            <div key={i} className="group bg-card rounded-2xl overflow-hidden border border-border hover:border-primary/30 transition-all">
              <div className="relative h-48 overflow-hidden">
                <Picture src={card.img} alt={card.name} width={800} height={600} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" decoding="async" />
              </div>
              <div className="p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {[IMG.glossCard, IMG.mattCard, IMG.spotUvCard, IMG.velvetCard, IMG.goldFoilCard, IMG.syntheticCard, IMG.premiumCards, IMG.stackedCards].map((img, i) => (
            <div key={i} className="rounded-xl overflow-hidden group">
              <Picture src={img} alt="Visiting card sample" width={400} height={300} className="w-full h-48 object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" decoding="async" />
            </div>
          ))}
        </div>
      </div>
