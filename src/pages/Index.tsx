...
// ─── BRAND ──────────────────────────────────────────────────────
const C = {
  red: "#8B0000", maroon: "#5C0000", gold: "#FFD700", goldDark: "#B8860B",
  saffron: "#E65100", teal: "#00695C", ivory: "#FFFDF7", dark: "#1A1A1A",
  darkAlt: "#111111", wa: "#25D366", cream: "#FFF8F0",
};
  ...
  return (
    <div id="top" style={{ fontFamily: PP, color: C.dark, margin: 0, padding: 0, background: C.ivory, overflowX: "hidden" }}>
      <style>{`
        ...
        @keyframes borderGlow{0%,100%{box-shadow:0 0 5px rgba(255,215,0,0.3)}50%{box-shadow:0 0 20px rgba(255,215,0,0.6),0 0 40px rgba(139,0,0,0.2)}}
      ...
      <nav style={{
        ...
        background: scrolled ? "rgba(26,26,26,0.85)" : "rgba(139,0,0,0.92)",
      ...
      <div style={{ position:"relative",minHeight:"100vh",overflow:"hidden",background:C.dark }}>
        ...
        <div style={{ position:"absolute",inset:0,background:"linear-gradient(135deg,rgba(139,0,0,0.92) 0%,rgba(92,0,0,0.85) 30%,rgba(26,26,26,0.7) 60%,rgba(0,0,0,0.3) 100%)",zIndex:2 }}/>
        {/* Background image */}
        {SLIDES.map((s,i)=>(
          ...
          <div className="grid-3col" style={{ display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:20,maxWidth:1200,margin:"0 auto" }}>
            {CATEGORIES.map((cat,i)=>(
              <div key={i} style={{
                ...
                <div style={{ position:"absolute",inset:0,background:hoverCat===i?`linear-gradient(135deg,rgba(139,0,0,0.9),rgba(92,0,0,0.85))`:"linear-gradient(0deg,rgba(0,0,0,0.7) 0%,rgba(0,0,0,0) 60%)",transition:"all 0.4s" }}/>
            ...
            {PRODUCTS.map((p,i)=>(
              <div key={i} style={{
                ...
                <div style={{ position:"relative",height:200,overflow:"hidden" }}>
                  ...
                  <div style={{ position:"absolute",inset:0,background:hoverProd===i?"linear-gradient(0deg,rgba(139,0,0,0.4),transparent)":"none",transition:"all 0.3s" }}/>
                ...
                <div style={{ padding:20 }}>
                  ...
                  <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center" }}>
                    ...
                    <button onClick={()=>setModal("cart")} style={{ background:`linear-gradient(135deg,${C.red},${C.maroon})`,color:"#fff",border:"none",padding:"9px 20px",borderRadius:30,cursor:"pointer",fontFamily:PP,fontSize:12,fontWeight:600,boxShadow:"0 4px 15px rgba(139,0,0,0.2)" }}>Order Now</button>
                  </div>
                </div>
              </div>
            ))}
      ...
      <div style={{ height:420,overflow:"hidden",position:"relative",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center" }}>
        ...
        <div style={{ position:"absolute",inset:0,background:`linear-gradient(135deg,rgba(139,0,0,0.85),rgba(92,0,0,0.7),rgba(26,26,26,0.6))` }}/>
      ...
      <Section>
        <div id="contact" className="contact-grid" style={{ display:"grid",gridTemplateColumns:"1fr 1fr",minHeight:600,maxWidth:1400,margin:"0 auto" }}>
          ...
          <div style={{ background:C.ivory,padding:"56px 48px",display:"flex",flexDirection:"column",justifyContent:"center",borderRadius:"0 20px 20px 0" }}>
            ...
            {["Full Name","Phone (+91)","Email Address"].map(ph=>(
              <input key={ph} type="text" placeholder={ph} style={{ border:"2px solid #eee",borderRadius:10,padding:"12px 16px",marginBottom:12,fontSize:14,fontFamily:PP,width:"100%",outline:"none",background:"#fff",transition:"border 0.3s" }}/>
            ))}
            <select style={{ border:"2px solid #eee",borderRadius:10,padding:"12px 16px",marginBottom:12,fontSize:14,fontFamily:PP,width:"100%",background:"#fff" }}>
            ...
            <textarea rows={3} placeholder="Describe your requirements" style={{ border:"2px solid #eee",borderRadius:10,padding:"12px 16px",marginBottom:16,fontSize:14,fontFamily:PP,width:"100%",resize:"vertical",outline:"none",background:"#fff" }}/>
            <button style={{ width:"100%",padding:"14px",background:`linear-gradient(135deg,${C.red},${C.maroon})`,color:"#fff",border:"none",borderRadius:30,fontFamily:PP,fontWeight:700,fontSize:15,cursor:"pointer",boxShadow:"0 8px 25px rgba(139,0,0,0.2)",transition:"all 0.3s" }}>Send Enquiry</button>
          </div>
        </div>
      </Section>
